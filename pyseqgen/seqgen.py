        ...
        if isinstance(self.trees, list):
            ...
            for idx, tree in enumerate(self.trees):
                if self.num_sites_per_tree or self.rel_rates_per_tree:
                    ...
                    parts = ""
                input.append("%s%s;" % (parts, tree.compose_newick()))
            return '\n'.join(input)
        else:
            return ";".join([t.compose_newick() for t in self.trees]) + ";"
#             
...
#     source_dataset = dataio.dataset_from_file(tree_filepath, format=format_desc)
    ...
    if source_dataset and len(source_dataset.trees_blocks) > 0 and len(source_dataset.trees_blocks[0]) > 0:
        seqgen.trees = source_dataset.trees_blocks[0]
    else:
